...
export default function DirectSale() {
    ...
    const [orderStorage, setOrderStorage] = useState(
        localStorage.getItem("directSaleOrder")
    );
        ...
        if (!orderId) {
            ...
            if (orderParam) {
                ...
                confirm({
                    ...
                    onCancel() {
                        localStorage.removeItem("directSaleOrder");
                    },
                });
            }
        }
    ...
    useEffect(() => {
        if (isSuccessMutation || isUpdateSuccessMutation) {
            ...
            window.location.href = `/orders/directSale/checkout?order=${order}`;
        }
    ...
    useEffect(() => {
        if (isSuccessStoredMutation || isUpdateSuccessStoredMutation) {
            ...
            localStorage.setItem("directSaleOrder", order);
            setOrderId(order);
        ...
        if (isSuccessOrder && order) {
            const products = order.order_products.map((item) => {
                return {
                    ...
                    unit: item.unit,
                    bms_product_batch: item.bms_product_batch,
    ...
    const addCartProduct = ({ quantity, product, volume, batchId }) => {
        setCartProducts([
            ...cartProducts,
            {
                product: product,
            ...
            if (cartProduct.product.id === product.product.id && productIndex === index) {
                if (action === "increase") {
                    ...
                    if (
                        ...
                        product.quantity = parseFloat(inputValue).toFixed(2);
                    }
                }
            }
    ...
    const sendToCheckout = (saveInStorage = false) => {
        ...
        const orderProducts = cartProducts.map((cartProduct) => {
            ...
            const conversion = quantity;
            const discountAux = discount > 0 ? discount : 0;
            const correctionPricePercentageAux = correctionPricePercentage > 0 ? correctionPricePercentage : 0;
            ...
            return {
                ...
                quantity,
                price,
                correctionPrice: correctionPricePercentageAux,
                discountAux,
                bms_product_batch
            };
        });
