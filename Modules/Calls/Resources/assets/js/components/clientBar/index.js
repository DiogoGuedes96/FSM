...
export default function clientBar({ open, onClose, client }) {
    ...
    const { data: lastSalesAndProducts, refetch: refetchLastSalesAndProducts } = useQuery(
        ["getAllClients", client?.client?.id, startDate, endDate],
        () => {
          return getLastSalesAndProducts(client?.client?.id, startDate, endDate)
        },
        {
    ...
    useEffect(() => {
        if (client && client?.client?.primaveraClient) {
            refetchLastSalesAndProducts();
            let trueClient = client?.client;
            if(trueClient?.primaveraClient?.invoices?.length > 0){

                setLastInvoice(trueClient?.primaveraClient?.last_invoice ? trueClient?.primaveraClient?.last_invoice : null);
                setPendingInvoices(
                    trueClient?.primaveraClient?.invoices ? trueClient?.primaveraClient?.invoices : []
                );
            }
        }
    }, [client]);
    ...
    const addClientToLocalStorage = (client) => {
        if (client?.client) {
            ...
            localStorage.setItem(
                "callerPhone",
                client?.call?.callee_phone
                    ? client?.call?.callee_phone
                    : client?.call?.caller_phone
            );
        } else {
            localStorage.setItem(
                "callerPhone",
                        ...
                        <div style={{ paddingTop: 32 }}>
                            ...
                            <List style={{ marginTop: "1rem" }}>
                                ...
                                <ClientItemBar
                                    ...
                                    value={client?.client?.primaveraClient?.sumInvoiceDebt?.toLocaleString(
                                        "pt-PT",
                                        {
                                            style: "currency",
                                            currency: "EUR",
